...
The committed approach (MVP1): a new **AI Motion Graphic** page, peer to Projects / Storyboard / Generate AI, where a Creator describes a graphic, an AI authors a reusable code-backed **Motion Graphic**, previews it live, and refines it through a persistent **chat history** that remains the graphic's editable source forever. The graphic is then reusable as a **media asset** (peer to image/video/audio) attached to **storyboard blocks**, carrying a fixed **duration**. Reuse in MVP1 is by duplication; a typed **props schema** and version-capable shape are laid into the model from day one so that insertion-point parameterization and version-pinning can be added later without a rewrite. Graphics are strictly per-Creator (no cross-account sharing in MVP1), and AI calls are gated by the existing **cost-estimate + confirm** mechanism.

Traceability context: the cost gate reuses the instrument-only, server-re-validated estimate pattern already in the storyboard pipeline (no credit ledger). The asset-attachment model mirrors the existing storyboard media pivot, extended with a new motion-graphic kind. The owner explicitly chose **native live-code rendering at export** (ideal quality + future live re-parameterization) over pre-rendering to an alpha video; this makes safe, deterministic execution of AI-authored code the feature's defining technical risk and is escalated to §6.1 and §8 for an architecture decision.
...
**Then** the system authors a Motion Graphic, records it under the Creator's account with a fixed duration, and shows it ready to preview

### AC-02 (US-03) — happy path

**Given** a Creator who owns a successfully generated Motion Graphic
**When** the Creator opens the graphic
**Then** the system plays the graphic back full-screen in real time and shows its authoring chat below
...
**Given** an authenticated Creator on the AI Motion Graphic page
**When** the Creator submits an empty or unintelligibly short description
**Then** the system declines to start the generation and explains in plain language that a meaningful description is required

### AC-06 (US-02) — error / generation cannot produce a usable graphic

**Given** a Creator whose confirmed generation produced a graphic that fails to run in the live preview
**When** the system finishes processing that attempt
**Then** the system marks the graphic as not usable, explains that the attempt did not produce a working graphic, and lets the Creator retry or refine rather than presenting a broken preview

### AC-07 (US-05) — authorization

**Given** a Creator who is not the owner of a particular Motion Graphic
**When** that Creator attempts to open, preview, or continue the chat of that graphic
**Then** the system denies access and does not reveal the graphic's existence or content, because graphics are private to their owner
...
**When** the request proceeds with a cost that does not match what the system independently re-computes
...
**Then** the system creates an independent copy owned by the same Creator, with its own chat history seeded from the original, that can be refined without affecting the original
...
|---|---|---|
| Latency p95 — live preview ready after code change | ≤ 1500 ms from code received to first rendered frame | client preview-render timing metric |
...
| Render parity (preview vs server export) | 100% of ready graphics render frame-identical in browser preview and server export | frame-diff check in CI on a fixture set |
| Generated-code execution isolation | Generated code cannot read the Creator's session, reach hosts outside an explicit allowlist, or access server secrets/filesystem/internal network | sandbox conformance test suite (browser + server) |
| Server render execution budget | Per-graphic render bounded by a CPU/time/memory cap; an over-budget graphic is aborted, never blocks the shared export queue | render-worker resource limit + abort metric |
...
- **AuthZ/AuthN impact:** authenticated Creators only; every read/write of a Motion Graphic, its chat, and its storyboard attachments is filtered by the owning account. No cross-account access path exists in MVP1. New capability: authoring and executing code-backed graphics.
- **Abuse cases:**
  - **Browser code exfiltration / XSS:** AI-authored (or prompt-injected) code attempts to read the session, cookies, or beacon data to an external host during preview → execution is isolated so generated code has no access to the Creator's session or non-allowlisted network; deny by construction.
  - **Server-side RCE / SSRF at export:** because export executes the live code server-side, authored code attempts to reach internal services, cloud metadata, secrets, or the filesystem → server execution runs isolated with no secret/filesystem/internal-network reach; deny by construction. (This is the feature's sharpest risk and the subject of an §8 architecture decision.)
  - **Resource-exhaustion DoS:** code with infinite loops or huge allocations freezes the editor tab or pins the render fleet → execution budget caps abort the graphic; a single bad graphic cannot starve the shared queue.
  ...
  - **Non-deterministic render abuse / drift:** time/random-driven code makes export diverge from the approved preview → the deterministic-render rule blocks the graphic from reaching ready state.
- **Security review:** **Required** — this feature introduces a new trust boundary (executing untrusted, AI-authored code in both the browser and the server renderer).

## 7. Metrics / KPIs

- **Adoption — projects using a Motion Graphic** — baseline: 0, target: ≥ 30% of new projects contain at least one attached Motion Graphic within 60 days of launch.
...
- [ ] What is the isolation mechanism for executing AI-authored code **server-side at export** with no secret/filesystem/internal-network reach, given the owner chose native live-code rendering? Default now: treat all generated code as untrusted and isolate execution in both browser and server. — owner: Tech Lead + Security Lead, due: before sdd:tasks (needs an ADR in sdd:design)
- [ ] What is the allowed import/runtime surface for generated code (which modules/APIs may a graphic use)? Default now: a minimal allowlist (rendering runtime + schema lib only), reject anything else at authoring time. — owner: Tech Lead, due: before sdd:tasks (ADR)
- [ ] How is asset-rot handled when the underlying rendering runtime version changes and older saved graphics may stop rendering? Default now: pin the runtime version and accept that re-validation/migration of old graphics is a later concern. — owner: Tech Lead, due: before sdd:design completes
- [ ] How is preview↔export frame parity enforced and detected before a Creator commits a graphic to a finished project? Default now: a deterministic-render rule + a CI fixture parity check (§6 NFR). — owner: Tech Lead, due: before sdd:plan-tests
