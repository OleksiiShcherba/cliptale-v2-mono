...
- **Save indicator** — існуючий статусний напис «Saving…/Saved just now» у верхній панелі сторінки storyboard, що відображає стан lightweight autosave, включно зі станом «не збережено» при збої збереження. NOT checkpoint countdown bar (той показує час до наступного checkpoint save і є окремим елементом).
