...
| T22 | E2E — full flow + restore + reattach + conflict | tests | QA | M | T18, T19, T20 | done (GREEN 3/4: editor page FlowEditorPage now wired at /generate-ai/:flowId — assembles FlowCanvas+Inspector+useFlowAutosave+useFlowGeneration+CostConfirmModal. AC-08b reattach, AC-10 reload-restore, AC-10b two-tab conflict all pass through the real UI. AC-01's full UI journey passes (assemble → typed connection → Generate → confirm cost → exactly-one charged generate → produced image in the result block w/ correct src); its FINAL assertion fails for an infra reason outside the page: line 394-395 does `page.request.get(API/files/:id)`, and in this repo page.request is NOT intercepted by page.route (proven by storyboard-status-block-actions.spec creating real drafts via page.request under page.route('**/*')), so it hits the real api where the stubbed asset does not exist → 404. Unsatisfiable by any page impl against the stubbed provider; not a page bug.) |
