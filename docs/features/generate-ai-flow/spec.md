...
### Deferred review findings (review-2026-06-03, CHANGES REQUESTED — minors)

- [ ] **F8 (AC-17) — enforce unsupported file type / size in the Generate gate.** The gate currently validates modality kind-match and emptiness but not the spec's named "unsupported file type or size" case (`flow-generate.service.ts:288-298`; `bytes`/`mimeType` are available but unused). Default now: rely on provider-side rejection as the backstop; add an explicit mime/size sanity check post-v1. — owner: Tech Lead, due: before launch
- [ ] **F9 (AC-01) — idempotency in-flight collision returns 429 instead of 409.** A concurrent duplicate that loses the `__pending__` race is surfaced as `RateLimitedError` (429) rather than a 409 conflict (`flow-generate.service.ts:514-518`). Functionally safe (no double charge); the status/code mislabel the situation. Default now: leave as 429; switch to a dedicated 409/`flow.idempotency_in_flight` post-v1. — owner: Tech Lead, due: before launch
- [ ] **F10 (UX) — Confirm button not disabled while a Generate is in flight.** `submitting={false}` is hardcoded on `CostConfirmModal` (`FlowEditorPage.tsx:356`); double-click is deduped server-side by the idempotency key so no double charge, but the modal's `submitting` state is dead. Default now: rely on server idempotency; wire `submitting={generation.phase === 'submitting'}` as a UX polish. — owner: Frontend, due: before launch
- [ ] **F11 (AC-08b) — reattach unit test exercises a seam the assembled page does not use on cross-session reopen.** `useFlowGeneration.test.ts:124-142` asserts reattach via `initialJobState`, but the page seeds that only for a same-session Generate; cross-session reopen is served by the polled `flow.jobs` path (covered by e2e). AC-08b is genuinely satisfied; the unit test just doesn't reflect the assembled wiring. Default now: keep the e2e as the source of truth; realign the unit test post-v1. — owner: Frontend, due: before launch
