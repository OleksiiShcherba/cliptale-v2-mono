...
| **AC-01** — run history (U5) | a regeneration appends a new result block; each block keeps its own run | component + e2e-through-UI | A second Generate ADDS a result block (stacked above, bound via `params.jobId`); the prior block — incl. a failed run's — is retained; on reload each block shows the output of the run that produced it (legacy unbound blocks fall back to the latest run). |
