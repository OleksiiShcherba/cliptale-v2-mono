...
| ~~Open architectural decision: reference-блок застряг in-progress (job lost, completion ніколи не fire) тримає гейт закритим~~ **Resolved 2026-06-10 (implement, default): no reaper — out of scope.** Known limitation: відновлення лише наявними delete-block / re-run-flow; revisit якщо deadlock-інциденти > 0 (spec §8 OQ-2, KPI §7) | Closed | — | Tech Lead |
| ~~Open architectural decision: mid-run регенерація reference-блока під час повного scene-пасу — re-validate per scene чи one-shot start check~~ **Resolved 2026-06-10 (implement, default): one-shot check at start.** Known limitation: зміни outputs під час in-flight пасу підхоплюються лише наступним стартом (spec §8 OQ-3) | Closed | — | Tech Lead |

**Accepted debt (acceptable in v1, plan to fix later):**
- Legacy `storyboard_illustration_references` рядки лишаються в БД й ігноруються на читанні (ADR-0004); `DROP TABLE` стейджено в `migrations/_deferred/` і **не промоутиться цією фічею** — окремий PR після KPI-вікна (`principal_image_generations = 0` протягом 7 днів post-rollout, spec §7).
