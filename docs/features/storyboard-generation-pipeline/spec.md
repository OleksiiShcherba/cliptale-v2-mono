...
- [ ] **Scene blocks lost on cancel → re-trigger → reload churn (review r8 F4, AC-01/AC-02).** A live edge run that cancelled scene generation, re-triggered it, and reloaded mid-phase ended with **0 `storyboard_blocks` (block_type='scene')** despite `scene_status='completed'` and a cast proposal that had shown non-empty scenes — so any downstream phase then 422s `pipeline.scenes_required`. A clean run (no cancel/reload) keeps its scene blocks intact, so this is specific to the cancel→re-trigger→reload interaction. Suspected: a stray cancelled plan job racing the re-triggered job's `materializeScenePlan` (delete-then-insert), and/or the client canvas autosave (`useStoryboardAutosave` → `replaceStoryboard`) clobbering server-materialized scene blocks with a stale/empty canvas after reload. Root cause not yet isolated. Default now: investigate as a dedicated task (reproduce → decide whether materialization must be guarded against concurrent jobs and/or autosave must not delete server-owned scene blocks). — owner: Tech Lead, due: before merge
</content>
