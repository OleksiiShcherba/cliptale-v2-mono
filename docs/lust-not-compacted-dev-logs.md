...
### Task: Guardian Recommendations Cleanup
**Subtask:** subtask-3-e2e-file-length — Resolve §9.7 300-line cap violation for e2e storyboard spec files

**What was done:**
- Added an explicit E2E spec file exemption clause to §9.7 (File length) in `docs/architecture-rules.md`
- The exemption covers all `e2e/*.spec.ts` Playwright files; `e2e/helpers/` retains the 300-line cap
- The quality gate substitution is documented: internal structure (one `test.describe` per spec file, shared reusable helpers in `e2e/helpers/`) rather than line count
- Updated the "Split test file naming convention" heading to clarify it applies to unit and integration test files only, not E2E specs

**Files created or modified:**
- `docs/architecture-rules.md` — added E2E exemption sub-section under §9.7 File length; refined the split-test-file naming convention heading

**Tests written:**
- No automated tests required — this is a documentation-only rule change
- Manual conformance verified: both violating files (`e2e/storyboard-canvas.spec.ts` 427L and `e2e/storyboard-history-regression.spec.ts` 329L) each have exactly one `test.describe` block, satisfying the new quality gate

**Notes:**
- Splitting the files along describe boundaries was considered but rejected: each spec file contains exactly one describe block with no natural split point, and helper functions are already correctly placed in `e2e/helpers/` or are spec-specific (e.g. `installCorsWorkaround` which is specific to the storyboard canvas deployment setup)
- The exemption is scope-bounded to `e2e/` root spec files only; helper files in `e2e/helpers/` keep the 300-line cap since they are pure utility code with no test boilerplate overhead

**Completed subtask from active_task.md:**
<details>
<summary>Subtask: subtask-3-e2e-file-length</summary>

Resolve §9.7 300-line cap violation — e2e/storyboard-canvas.spec.ts (427L) and e2e/storyboard-history-regression.spec.ts (329L) exceed the limit. Either split each file along internal describe boundaries, or add an explicit E2E exemption clause to §9.7 in docs/architecture-rules.md to match current practice.
...
design-reviewer notes: Reviewed on 2026-04-23. Documentation-only rule update to architecture-rules.md §9.7 (E2E spec file exemption). Zero UI/design surface. Automatic pass.

playwright-reviewer notes: Documentation-only change (zero code, zero UI surface). E2E spec exemption clause added to §9.7; no executable code or visual changes. Automatic YES per documentation-only pattern.

---

## [2026-04-23]

### Task: Guardian Cleanup Batch 4
**Subtask:** subtask-4-stale-known-issues — Remove stale Class A Known Issue entry

**What was done:**
- Removed the stale "Class A (pre-existing DEV_AUTH_BYPASS user-mismatch / dropped-table refs)" bullet from the Known Issues / TODOs section in `docs/development_logs.md`. The entry listed `renders-endpoint.test.ts` and `versions-list-restore-endpoint.test.ts` as having outstanding issues; both are now passing (renders passes 10/10; versions was fixed in a prior subtask). The bullet is no longer accurate.

**Notes:**
- No code files were modified — this is a documentation cleanup only.
- No tests are required for a documentation-only change.

**Completed subtask from active_task.md:**
<details>
<summary>Subtask: subtask-4-stale-known-issues</summary>

Remove stale Class A entry from docs/development_logs.md Known Issues — the entry for renders-endpoint.test.ts says it has DEV_AUTH_BYPASS / dropped-table issues, but the test currently passes 10/10. Drop the bullet to keep the Known Issues list accurate.
...
design-reviewer notes: Reviewed on 2026-04-23. Documentation-only cleanup (removed stale Known Issue bullet from development_logs.md). Zero UI/design surface. Automatic pass.

playwright-reviewer notes: Documentation-only change (no code, no UI surface). Removed stale Class A Known Issue bullet from development_logs.md. Zero E2E surface — automatic pass per documentation-only pattern.
