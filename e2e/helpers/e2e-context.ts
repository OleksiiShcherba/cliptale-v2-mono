/**
 * Shared access to the projectId created by global-setup.
 *
 * Specs import `readE2eProjectId()` to navigate to `/editor?projectId=<id>`
 * so every test reuses the same empty project — no repeated POST /projects,
 * no accumulating state, no cross-test race.
 */
...
export const E2E_CONTEXT_PATH = path.resolve(
  __dirname,
  '../../test-results/e2e-deploy-context.json',
);
