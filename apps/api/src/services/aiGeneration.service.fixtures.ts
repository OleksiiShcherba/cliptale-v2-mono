 ...
 *
 * Registers module-level mocks for the repository + queue + asset-resolver
...
export const getAssetByIdMock =
  assetRepository.getAssetById as unknown as Mock;
...
export const TEST_PROJECT = 'proj-123';
export const TEST_ASSET_ID = 'asset-fixture-001';
export const FIXED_JOB_ID = 'job-fixed-000';
export const FIXED_PRESIGNED_URL = 'https://s3.example.com/presigned-foo';

/** Builds a minimal `Asset` row for resolver tests. Override any fields per-test. */
export function makeAssetRow(overrides: Partial<Asset> = {}): Asset {
  return {
...
export function resetMocks(): void {
  ...
  // Default: no asset row; tests that exercise the resolver must arrange a
  // row via `getAssetByIdMock.mockResolvedValue(makeAssetRow({ ... }))`.
